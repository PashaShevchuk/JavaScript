...
// логировать их, а также поддерживать отмену операций. Убирает прямую зависимость между объектами, вызывающими операции,
// и объектами, которые их непосредственно выполняют. Позволяет реализовать простую отмену и повтор операций. Позволяет
// реализовать отложенный запуск операций. Позволяет собирать сложные команды из простых. Реализует принцип открытости/закрытости.
...
class Command {
  constructor(subject) {
    ...
    this.commandsExecuted = []; // для зберігання команд які були викликані
  }
...
const c = new Command(new MyMath(3));

console.log(c.execute('square')); // 9
console.log(c.execute('cube'));   // 27
console.log(c.execute('cube'));   // 27

console.log(c.commandsExecuted); // [ 'square', 'cube', 'cube' ]
...
// конкретным классам. Программа будет состоять из целой сети Наблюдателей, не имея центрального объекта-Посредника.
{
  ...
  pasha.send('Hello!', lena);       // Pasha => Elena: Hello!
  lena.send('Hello hello!', pasha); // Elena => Pasha: Hello hello!
  igor.send('Vsem privet');         // Pasha: Vsem privet // Elena: Vsem privet
